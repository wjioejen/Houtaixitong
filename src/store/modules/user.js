...
const getDefaultState = () => {
  return {
    ...
    // 按钮权限的信息
    buttons: [],
    // 对比之后【项目中已有的的异步路由，与服务器返回的标记信息进行对比最终需要展示的理由】
    resultAsyncRoutes: [],
    // 用户最终需要展示全部路由
    resultAllRputes:[]
  }
...
const mutations = {
  ...
  SET_USERINFO: (state, userInfo) => {
    ...
    // 用户头像
    state.avatar = userInfo.avator
    // 菜单权限标记
    state.rootes = userInfo.routes
    //按钮权限标记
...
const computedAsyncRoutes = (asyncRoutes, routes) => {
  ...
  return asyncRoutes.filter(item => {
    //数值当中没有这个元素返回索引值-1，如果有这个元素返回的索引值一定不是-1
  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo(state.token).then(response => {
        // 获取用户信息:返回的信息包含：用户名name、用户头像avator、routes【返回的标志：不同的用户应该展示哪些菜单的标记】，roles（用户角色的信息）
