  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU图片">
        <!-- 上传图片 action图片上传的地址，需要加上dev-api也就是服务器地址
             否则会有跨域问题
...
export default {
  ...
  data() {
    return {
      ...
      // 添加spu，如果是添加spu的时候并没有向服务器发请求，数据收集到【spu],收集数据的时候有哪些字段，看文档
      spu: {
        // 三级分类的id
        category3Id: 0,
        // 描述
      ...
      saleAttrList: [], //销售属性的数据（项目的全部的销售属性）
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      // file参数：代表的是删除的那张图片
      // fileList:照片墙删除某一张图片以后，剩余的其它的图片
      console.log(file, fileList);
      // 收集照片墙图片的数据
      // 对于已有的图片【照片墙中显示的图片：有name、url字段的】,因为照片墙显示数据务必有这两个字段
      // 对于服务器而言，不需要有name、url字段，将来对于已有的图片的数据交给服务器的时候，需要处理的
    ...
    async initSpuData(spu) {
      ...
      if (spuImageResult.code == 200) {
        ...
        // 由于照片墙显示图片的数据需要修改数据，数组里面的元素需要有name与Url字段
        // 需要把服务器返回的数据进行修改
    ...
    addSaleAttrValue(row) {
      //点击销售属性值当中添加按钮的时候，需要有button变为input，通过当前销售属性的einputVisible控制
    ...
    handleInputConfirm(row) {
      ...
      // 属性值不能重复  every返回的是一个布尔值false 这里也可以用some
    ...
    async addOrUpdateSpu() {
      ...
      // 发请求
      let result = await this.$API.spu.reqAdOrdUpdateSpu(this.spu);
      console.log(result);
      if (result.code == 200) {
        // 提示
  ...
  computed: {
    ...
    unSelectSaleAttr() {
      ...
      // 当前SPU所有的属于自己的销售属性SPU.spuSaleAttrList
      // 数值的过滤方法，可以从已有的数组当中过滤用户需要的元素，并未返回一个新的数组
